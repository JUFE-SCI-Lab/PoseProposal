...
class DeformableEncoder(nn.Module):
    ...
    def forward(self, srcs, pe_layer):
        src_flatten = []
        lvl_pos_embed_flatten = []
        spatial_shapes = []
        for lvl, src in enumerate(srcs):
            bs, c, h, w = src.shape
            masks = src.new_zeros((bs, h, w)).to(torch.bool)
            pos_embed = pe_layer(masks)
        ...
        split_size = [None] * self.num_encoder_layers
        for i in range(self.num_encoder_layers):
            if i < self.num_encoder_layers - 1:
                split_size[i] = level_start_index[i + 1] - level_start_index[i]
            else:
                split_size[i] = memory.shape[1] - level_start_index[i]
        memory = torch.split(memory, split_size, dim=1)

        out = []
        for i, z in enumerate(memory):
