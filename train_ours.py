...
from propformer import *


def parse_args():
    parser = argparse.ArgumentParser(description='Train a pose model')
    parser.add_argument('--batch-size', default=4, type=int)
    parser.add_argument('--load-from', default=None, help='checkpoint file to load (train from scratch)')
...
def main():
    ...
    if cfg.checkpoint_config is not None:
        cfg.checkpoint_config.meta = dict(mmpose_version=__version__ + get_git_hash(digits=7),
                                          config=cfg.pretty_text, )

    logger.info(f'')
    logger.info(f'Total params: {sum([p.nelement() for p in model.parameters()]) / 1e6:.2f}M')
    for name in ['backbone', 'backbone_projs', 'predict_heads']:
        if hasattr(model, name):
            logger.info(f'{name}: {sum([p.nelement() for p in eval(f"model.{name}").parameters()]) / 1e6:.2f}M')
        else:
            logger.info(f'No module named {name}.')
    logger.info(f'')

    train_cid_num = len(train_datasets[0].valid_class_ids)
    train_img_num = len(train_datasets[0].paired_samples)
    logger.info(f'There are {train_cid_num} base classes, with {int(train_img_num / train_cid_num)} resampled imgs per')
    val_cid_num = len(val_dataset.valid_class_ids)
    val_img_num = len(val_dataset.paired_samples)
    logger.info(f'There are {val_cid_num} novel classes, with {int(val_img_num / val_cid_num)} imgs per')

    model.all_cat_info = val_dataset.cats
