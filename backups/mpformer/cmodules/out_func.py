...
def obtain_inference_result(meta_to_kp, gtdict_s,
                            meta_point_q, ref_point_q, invisible_score=0.5):
    all_res = {}
    for b, m2k in enumerate(meta_to_kp):
        visible_s = gtdict_s['gtvisibles'][b, :, 0].bool()
        for l, pred_point in enumerate(meta_point_q[b]):
            full_res = torch.ones_like(gtdict_s['gtpoints'][b]) * invisible_score
            full_res[visible_s] = pred_point[m2k]
            key = f'meta_{l}'
