...
class PGEmbeddingDecoder(nn.Module):
    def __init__(self, decodercfg):
        super().__init__()
        self.decodercfg = decodercfg
        self.out_layer_num = decodercfg.out_layer_num
        self.in_layer_num = decodercfg.in_layer_num

        self.global_dim = 256
        self.enc_layers = nn.ModuleList([EncoderLayer(n_levels=1,
                                                      n_heads=decodercfg.n_heads,
                                                      d_ffn=decodercfg.d_ffn,
                                                      n_points=decodercfg.n_points)
                                         for _ in range(self.in_layer_num)])

        self.point_heads = nn.ModuleList([TokenDecodeMLP(in_channels=self.global_dim,
                                                         hidden_channels=self.global_dim,
                                                         num_layers=decodercfg.n_mlp)
                                          for _ in range(self.in_layer_num)])
    ...
    def forward(self, query_embed, featmaps, init_points=None):
        '''
            ...
            init_points: [B,K,2]
        '''
        raw_input_query_embed = query_embed
        if init_points is None:
            init_points = self.get_default_init_point(query_embed)

        refined_points_ml = []

        for out_l in range(self.out_layer_num):
            for in_l, featmap in enumerate(featmaps[:self.in_layer_num]):
                bs, c, h, w = featmap.shape
                spatial_shape = init_points.new((h, w))[None].long()
                level_start_index = init_points.new_zeros((1,))
                featmap_flatten = featmap.flatten(2).transpose(1, 2)
                query_pos = self.get_query_pos(init_points)
                query_embed = self.forward_self_att(query_embed, query_pos, in_l)

                if self.decodercfg.every_ipt_raw:
                    query_embed = query_embed + raw_input_query_embed

                refined_embed = self.enc_layers[in_l](query_embed, query_pos, featmap_flatten, init_points,
                                                      spatial_shape, level_start_index)
                refined_points = (inverse_sigmoid(init_points) + self.point_heads[in_l](refined_embed)).sigmoid()
                refined_points_ml.append(refined_points)
                init_points = refined_points
